    ...
    for (let i of members) {
        if (i.admin == 'admin' && i.id !== botNumber || i.admin == 'superadmin' && i.id !== botNumber) {
            ...
            if (user) {
                user.name = namanye
                user.premium = true
                user.premiumTime = (waktunya.toLowerCase() === "permanent" || waktunya.toLowerCase() === "unlimited") 
                    ? Infinity 
                    : Date.now() + toMs(waktunya)

                user.timeOrder = timeWib
                user.timeEnd = (waktunya.toLowerCase() === "permanent" || waktunya.toLowerCase() === "unlimited") 
                    ? "Unlimited" 
                    : moment(Date.now() + toMs(waktunya)).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm')
            } else {
                db.data.users[nomernya] = {
                    name: namanye,
                    premium: true,
                    premiumTime: (waktunya.toLowerCase() === "permanent" || waktunya.toLowerCase() === "unlimited") 
                        ? Infinity 
                        : Date.now() + toMs(waktunya),

                    timeOrder: timeWib,
                    timeEnd: (waktunya.toLowerCase() === "permanent" || waktunya.toLowerCase() === "unlimited") 
                        ? "Unlimited" 
                        : moment(Date.now() + toMs(waktunya)).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm')
                }
            }
...
• *Days:* ${waktunya.toLowerCase() === "permanent" || waktunya.toLowerCase() === "unlimited"
    ? "Unlimited"
    : conn.msToDate(toMs(waktunya))}

• *Countdown:* ${waktunya.toLowerCase() === "permanent" || waktunya.toLowerCase() === "unlimited"
    ? "Unlimited"
    : toMs(waktunya)}
• *Time order:* ${timeWib}
• *Time end:* ${waktunya.toLowerCase() === "permanent" || waktunya.toLowerCase() === "unlimited"
    ? "Unlimited"
    : moment(Date.now() + toMs(waktunya)).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm')}
`
...
Log(q.split("|")[0])
if (!waktunya) return setReply("Masukkan waktu\ns = detik\nh = jam\nd = hari\nGunakan 'permanent' atau 'unlimited' untuk masa aktif tanpa batas.")
