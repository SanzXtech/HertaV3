let handler = async (m, { conn, usedPrefix }) => {
  ...
  if (id in conn.mancing) {
    conn.reply(
      m.chat,
      `Selesaikan mancing ${conn.mancing[id][0]} Terlebih Dahulu`,
      m
    );
    throw false;
  }
  if (user.umpan == 0)
    return m.reply("Kamu Membutuhkan Umpan 🪱 Untuk Memancing!");
  ...
  if (user.stamina < 50)
    return m.reply(
      "Kamu Kelaparan & Tidak Punya Stamina , Kamu Harus Memiliki Setidaknya 50 Stamina ⚡ Untuk Memancing"
    );
  if (user.strength < 2)
    return m.reply(
      "kamu Terlalu Lemah Untuk Memancing Ikan , Kamu Harus Memiliki [ Ability Strength 2 💪🏻 ] Pergi Lah Berlatih Dengan Cara Ketik .latih"
    );
...
⚡ Stamina : ${energi}`.trim();
  ...
  } else m.reply(`Mohon Tunggu Selama ${timers} Sebelum Memulai mancing Kembali`);
};
