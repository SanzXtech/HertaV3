let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  if (global.db.data.users[m.sender].dragon < 1) {
    return m.reply(`Kamu tidak memiliki dragon saat ini. Silakan beli dragon di petshop atau coba gacha di *tbox* untuk mendapatkan dragon!`);
  }
  ...
  m.reply(
    `*Pet Kamu* (dragon ${global.db.data.users[m.sender].dragon}) Menantang dragonnya *${conn.getName(lawan)}* (dragon ${global.db.data.users[lawan].dragon}) Lagi Kelahi Rebutin Bini.\n\nTunggu ${lamaPertarungan} Menit Lagi Dan Lihat Siapa Yang Menang.`
  );
  ...
  let kesempatan = [];
  for (let i = 0; i < global.db.data.users[m.sender].dragon; i++) kesempatan.push(m.sender);
  for (let i = 0; i < global.db.data.users[lawan].dragon; i++) kesempatan.push(lawan);
  ...
  if (pointPemain > pointLawan) {
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${conn.getName(lawan)}*\n\n*Pet Kamu* (dragon ${global.db.data.users[m.sender].dragon}) Menang Melawan dragonnya *${conn.getName(lawan)}* (dragon ${global.db.data.users[lawan].dragon}) Karena dragon Kamu ${alasanMenang[Acakin(0, alasanMenang.length - 1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}`
    );
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${conn.getName(lawan)}*\n\n*Pet Kamu* (dragon ${global.db.data.users[m.sender].dragon}) Kalah Melawan dragonnya *${conn.getName(lawan)}* (dragon ${global.db.data.users[lawan].dragon}) Karena Pet Kamu ${alasanKalah[Acakin(0, alasanKalah.length - 1)]}\n\nUang Kamu Berkurang Rp. ${denda.toLocaleString()}`
    );
  } else {
